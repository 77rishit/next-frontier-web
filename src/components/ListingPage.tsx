...
export function ListingPage({type,items}:{type:"competition"|"workshop",items:readonly any[]}){
 ...
 return <SiteShell><PageHero code={comp?"02 / CYBER ARENA":"03 / FUTURE LAB"} title={comp?"Challenge":"Learn from"} accent={comp?"the limits.":"the future."} copy={comp?`${items.length} live challenges drawn directly from Techfest’s current official roster.`:`${items.length} current on-campus workshops with verified dates, fees and registration links.`} image={comp?"/src/assets/arena-world.jpg":"/src/assets/laboratory-world.jpg"}/>
